...
class HybridBackend:
    ...
    def curvature_diffusion(self, wave, curvature, knots, crystal):
              ...
              csum = (self.roll(conductivity, -1, 0) + self.roll(conductivity, 1, 0)
                      + self.roll(conductivity, -1, 1) + self.roll(conductivity, 1, 1) + 1e-8)
...
class VoidResonanceV3:
    ...
    def __init__(self, H=32, W=32, K=8, **kwargs):
        ...
        # --- State fields ---
        self.psi = self.bk.zeros_complex((H, W, K))        # Wave field
        self.V = self.bk.ones_float((H, W, K))             # Void (vacuum)
        self.R = self.bk.ones_float((H, W, K))             # Curvature
        self.knots = self.bk.zeros_float((H, W, K))        # Topological defects
        self.crystal = self.bk.zeros_float((H, W, K))      # Crystal density
        self.crystal_phase = self.bk.zeros_float((H, W, K))  # [NEW] Phase imprint
        self.echo = self.bk.zeros_float((H, W, K))         # Wound echo buffer
        self.stress = self.bk.zeros_float((H, W))          # Global stress
        self.boundary = self.bk.zeros_float((H, W, K))     # [NEW] Boundary tension

        self.num_actions = min(4, K)
        self.t = 0

    # ------------------------------------------------------------------
    # [NEW] Crystal Boundary Tension
    # ------------------------------------------------------------------
    def _crystal_boundary_tension(self):
              """
                      Compute tension at boundaries between crystal regions with
        different phase imprints. Uses circular (angular) distance
                so that phase wraps correctly.

                        Tension is nonzero only where BOTH the local cell and its
                                neighbors have significant crystal density.
                                        """
              bk = self.bk
              cp = self.crystal_phase
              cr = self.crystal

        # Circular phase distance to each neighbor
        ...
        # Boundary exists only where both sides are crystallized
    ...
    # ------------------------------------------------------------------
    # [NEW] Crystal Phase Dynamics: Fracture, Fusion, Nucleation
    # ------------------------------------------------------------------
    def _crystal_phase_dynamics(self, boundary, neighbor_crystal, idle):
              """
                      Three boundary-driven processes:

                              FRACTURE: High tension shatters crystal back to knots and
                                                releases stored phase energy as a wave burst.

                                                        FUSION:   Low tension (similar phases) strengthens and unifies
                                                                          adjacent crystal regions.

                                                                                  NUCLEATION: Boundary energy seeds new knot structures in
                                                                                                      nearby empty (non-crystal) space.
                                                                                                              """
              bk = self.bk

        # --- FRACTURE ---
              fracture = bk.maximum(boundary - self.fracture_threshold, 0)
        # Crystal shatters into knot debris
        shatter_amount = fracture * self.crystal * 2.0
        self.knots += shatter_amount
        self.crystal = bk.maximum(self.crystal - fracture * self.fracture_decay, 0)
        # Shattered crystal releases phase-encoded energy as wave burst
        burst = (fracture * np.exp(1j * self.crystal_phase)).astype(np.complex64)
        self.psi += burst * self.fracture_energy_release

        # --- FUSION ---
        low_tension = bk.maximum(self.fusion_phase_tolerance - boundary, 0)
        fusion = low_tension * self.crystal * neighbor_crystal
        self.crystal = bk.clamp(self.crystal + fusion * self.fusion_rate, 0, 1)

        # --- NUCLEATION ---
        # Boundary energy creates new knots in non-crystallized space
        empty_space = bk.maximum(1.0 - self.crystal, 0)
        viable = bk.maximum(self.V - 0.3, 0)  # Needs some void to nucleate
    ...
    def step(self, s_amp, s_ph=None):
        ...
        # ============================================================
        # 1. WOUND + RESONANCE CASCADE (from v2)
        # ============================================================
        s = np.zeros(self.K)
        ...
        # ============================================================
        # 5. GEODESIC HOLOGRAPHIC DIFFUSION (from v2)
        # ============================================================
        self.psi = ((1 - self.alpha) * self.psi
        ...
        # ============================================================
        # 7. CRYSTALLIZATION + PHASE DYNAMICS (EXTENDED in v3)
        # ============================================================

        # 7a. Crystal growth (from v2) + phase imprinting (NEW)
        new_crystal = bk.maximum(self.knots - self.crystal_threshold, 0) * 0.1
        forming = (new_crystal > 0.001).astype(np.float32)
        psi_angle = np.angle(self.psi)

        # Imprint wave phase at moment of crystallization
        # Already-crystallized regions keep their existing phase
        self.crystal_phase = (
                      self.crystal_phase * (1 - forming) + psi_angle * forming
        )
        self.crystal = bk.clamp(self.crystal * 0.998 + new_crystal, 0, 1)

        # 7b. Boundary tension computation (NEW)
        self.boundary, neighbor_crystal = self._crystal_boundary_tension()

        # 7c. Phase dynamics: fracture, fusion, nucleation (NEW)
        self._crystal_phase_dynamics(self.boundary, neighbor_crystal, idle)

        # 7d. Scar erosion (from v2)
        ...
        # ============================================================
        # 8. CURVATURE UPDATE (from v2)
        # ============================================================
        self.R = bk.clamp(
                      self.R + st3 * amp * self.plasticity + 0.005 * (1 - self.R),
                      0.1, 10
        )
...
def run_tests():
    ...
    # ------------------------------------------------------------------
    # T6: Behavioral Differentiation (from v2)
    # ------------------------------------------------------------------
    ...
    # ==================================================================
    # NEW v3 TESTS: Crystal Phase Dynamics
    # ==================================================================
    print("-" * 70)
    ...
    #     Crystals formed by stimulus A should carry a different phase
    #     than crystals formed by stimulus B.
    # ------------------------------------------------------------------
    ...
    for _ in range(80):
          ...
          # Compare phase imprints in channel 0 where crystal exists
    ...
    print(f"[PASS={pass7}] T7: Crystal Phase Imprinting "
                    f"(Different stimuli produce different crystal phases)")

    # ------------------------------------------------------------------
    # T8: Boundary Tension Emergence
    #     Feed two conflicting stimuli in sequence. Boundary tension
    #     should emerge where the two crystal territories meet.
    # ------------------------------------------------------------------
    e_bt = VoidResonanceV3(
              knot_threshold=0.3, crystal_threshold=0.1,
              fracture_threshold=10.0  # Disable fracture for this test
    )
    # Phase A territory
    for _ in range(60):
              e_bt.step(s_A, np.array([1.0, 0, 0, 0, 0, 0, 0, 0]))
          # Phase B territory (different phase angle)
          for _ in range(60):
                    e_bt.step(s_A, np.array([-2.0, 0, 0, 0, 0, 0, 0, 0]))
                pass8 = e_bt.boundary[:, :, 0].max() > 0.01
    results.append(pass8)
    print(f"[PASS={pass8}] T8: Boundary Tension Emergence "
                    f"(max tension={e_bt.boundary[:,:,0].max():.4f})")

    # ------------------------------------------------------------------
    # T9: Crystal Fracture
    #     When boundary tension is high enough, crystals should shatter.
    #     Compare crystal mass with low vs high fracture threshold.
    # ------------------------------------------------------------------
    e_frac = VoidResonanceV3(
              knot_threshold=0.3, crystal_threshold=0.1,
              fracture_threshold=0.05  # Easy to fracture
    )
    e_nofrac = VoidResonanceV3(
              knot_threshold=0.3, crystal_threshold=0.1,
              fracture_threshold=100.0  # Never fractures
    )
    for _ in range(60):
          ...
          for _ in range(60):
                ...
                # Fracture engine should have less crystal (some shattered)
    ...
    print(f"[PASS={pass9}] T9: Crystal Fracture "
                    f"(frac_crystal={c_frac:.2f} vs intact={c_nofrac:.2f})")

    # ------------------------------------------------------------------
    # T10: Crystal Fusion
    #      Crystals with similar phases should merge and grow stronger
    #      than crystals without fusion enabled.
    # ------------------------------------------------------------------
    ...
    # Same stimulus (same phase) repeatedly
    for _ in range(100):
    ...
    #      Conflicting crystal zones should generate new knots in
    #      surrounding empty space via nucleation.
    # ------------------------------------------------------------------
    ...
    print(f"[PASS={pass11}] T11: Boundary Nucleation "
                    f"(nucleated_knots={k_nuc:.2f} vs baseline={k_nonuc:.2f})")

    # ------------------------------------------------------------------
    # T12: Fracture Wave Burst
    #      When crystals shatter, the released energy should produce
    #      stronger wave activity than a non-fracturing system.
    # ------------------------------------------------------------------
    ...
    for _ in range(60):
          ...
          for _ in range(60):
                ...
                # After conflict, check wave energy
                psi_burst = np.abs(e_burst.psi[:, :, 0]).mean()
    psi_noburst = np.abs(e_noburst.psi[:, :, 0]).mean()
    pass12 = psi_burst > psi_noburst
    results.append(pass12)
    print(f"[PASS={pass12}] T12: Fracture Wave Burst "
                    f"(burst_psi={psi_burst:.4f} vs calm={psi_noburst:.4f})")
